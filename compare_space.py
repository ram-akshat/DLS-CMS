...
STREAM = [random.choice(VOCAB) for _ in range(STREAM_SIZE)]

# ── Helper: build + populate one instance ────────────────────────────────────

def run_cms(eps, dlt):
    obj = CountMinSketch(epsilon=eps, delta=dlt)
    for w in STREAM: obj.update(w)
    return obj
...
for name, runner in RUNNERS:
    obj = runner(EPS_FIXED, DLT_FIXED)
